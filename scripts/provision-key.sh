...
if [ -n "$MQTT_CONFIG" ]; then
    [ -f "$MQTT_CONFIG" ] || {
        echo "MQTT config is not a file: $MQTT_CONFIG" >&2
        exit 1
    }
...
if [ "$ENABLE_SSH" = 1 ]; then
    ...
    ssh-keygen -lf "$SSH_PUBLIC_KEY_FILE" >/dev/null 2>&1 || { echo "invalid SSH public key: $SSH_PUBLIC_KEY_FILE" >&2; exit 1; }
    mkdir -p "$MOUNTPOINT/steamlink/overlay/mnt/config/ssh"
    cp -L "$SSH_PUBLIC_KEY_FILE" "$MOUNTPOINT/steamlink/overlay/mnt/config/ssh/authorized_keys"

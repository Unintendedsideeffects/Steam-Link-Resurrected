...
field() {
    ...
    busybox httpd -d "$encoded" 2>/dev/null || true
}
...
[ "${REMOTE_USER-}" = admin ] || fail '401 Unauthorized' 'Authentication required'
...
if [ "${REQUEST_METHOD-GET}" = POST ]; then
    ...
    save)
        ...
        pass_n=$(printf '%s' "$NEW_PASS" | wc -c | tr -d ' '); [ "$pass_n" -le 63 ] || fail '400 Bad Request' 'Wi-Fi password must be at most 63 bytes'; [ "$pass_n" -eq 0 ] || [ "$pass_n" -ge 8 ] || fail '400 Bad Request' 'WPA Wi-Fi password must be at least 8 bytes'
        tmp="$CONF.$$"; {
            printf 'SETUP_SSID=%s\nSETUP_PASSWORD=%s\nWEB_SECRET=%s\nHOSTNAME=%s\nWIFI_SSID=%s\nWIFI_PASSWORD=%s\n' "$(get SETUP_SSID)" "$(get SETUP_PASSWORD)" "$(get WEB_SECRET)" "$NEW_HOST" "$NEW_SSID" "$NEW_PASS"
            printf 'ENABLE_SSH=%s\nENABLE_USBIP=%s\nENABLE_VIRTUALHERE=%s\nENABLE_MQTT=%s\nENABLE_DIAGNOSTICS=%s\n' "$(get ENABLE_SSH)" "$(get ENABLE_USBIP)" "$(get ENABLE_VIRTUALHERE)" "$(get ENABLE_MQTT)" "$(get ENABLE_DIAGNOSTICS)"
...
printf '<section><h2>Features</h2><p>SSH: %s (disabling SSH risks losing access)<br>USB/IP: %s<br>VirtualHere: %s<br>Home Assistant MQTT: %s<br>USB diagnostics: %s</p>' "$(get ENABLE_SSH)" "$(get ENABLE_USBIP)" "$(get ENABLE_VIRTUALHERE)" "$(get ENABLE_MQTT)" "$(get ENABLE_DIAGNOSTICS)"
